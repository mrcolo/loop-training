...
\title{\vspace{-2em}\textbf{Two CDJs Is All You Need:\\
Specialising a Latent Rectified-Flow Audio Model to One Room's Sound}}
...
all 1.453\,B transformer parameters on 26.4 hours of DJ sets recorded at one Los
Angeles venue. Its central finding is about \emph{which} published
...
average fit on one 24\,GB GPU (\S\ref{sec:memory}), and describe the corpus and the
energy gating a raw booth recording needs (\S\ref{sec:data}). (v) We argue that per-step training loss is
...
\paragraph{The corpus.} $26.4$ hours of continuous DJ sets recorded at
\textsc{2n1t3} (\url{https://2n1t3.com}), a Los Angeles event organiser. It is a
single stereo FLAC of $10.5$\,GB: not a curated dataset but a raw capture of what
came out of the booth across many nights, including the transitions, the dead air
between sets, and whatever the room sounded like at the time. That is the point.
The target is not a genre, it is one venue's material as it is actually played,
and the model is asked to continue it.

Two properties follow from the source being a recording rather than a library.
There are no track boundaries, so an excerpt may straddle a transition, which is
exactly the structure outpainting has to handle. And the loudness varies enormously
across the night, so a naive uniform sample teaches the model the average of
a club and an empty room.

\paragraph{Energy gating.} We index the source every $8$\,s for RMS and sub-bass
fraction, which takes $45$\,s for the whole $26.4$ hours, and sample only windows
whose mean clears a threshold. At $\text{rms} \geq 0.15$ this retains $95\%$ of
the material and removes the silence between sets. Without it the model drifts
quiet, which is a failure that looks like a modelling problem and is not.

\paragraph{Precomputed latents.} The autoencoder is $89\%$ of an online training
step, so encoding once buys roughly $7\times$ more optimiser steps for a fixed
time budget. The whole recording becomes a single float16 array of $256$ channels
at $10.77$\,Hz: $501$\,MB for $26.4$ hours, memory-mapped, from which windows are
cropped at arbitrary frame offsets. The alignment of musical structure therefore
still varies continuously between epochs; what is given up is waveform-domain
augmentation, which mask shape substitutes for.

Encoding runs at $23\times$ realtime on an RTX 3090 and must be done in
overlapping blocks with the seams trimmed, and with the autoencoder in
\texttt{eval()}: its \textsc{SoftNorm} bottleneck applies noise whose scale differs
by $50\times$ between train and eval mode, and getting that wrong is invisible
until sample quality is inexplicably poor.

\begin{algorithm}[t]
\caption{One training step}
\label{alg:step}
\begin{algorithmic}[1]
\State $\bz \gets$ \textsc{Window}(latents, $B$, $\tau \sim \{47,95,190,380\}$s) \Comment{memory-mapped}
\State $\bm \gets$ \textsc{Mask}($B$, $T$) \Comment{0.55 full / 0.10 segments / 0.35 causal}
\State $t \gets \sigma_L(1 - \mathrm{TLN}())$ \Comment{flipped, length-shifted}
...
\State $\varepsilon \gets \hat v - v^\star$
\State $\mathcal{L} \gets \dfrac{\|\varepsilon\odot(1-\bm)\|^2}{256\sum(1-\bm)}
                     + \dfrac{\|\varepsilon\odot\bm\|^2}{256\sum\bm}$ \Comment{generation $+$ context}
\State accumulate; every 4 microbatches: clip to $1.0$
\State Muon step on attention and feed-forward matrices; AdamW8bit on the rest
\State update the weight average, $\beta = 0.9995$
\end{algorithmic}
\end{algorithm}

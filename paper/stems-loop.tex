...
report the failure it produces in full (\S\ref{sec:checkpoint}, \S\ref{sec:results}).
...
% ==========================================================================
\section{Diagnosis of a faithful-looking failure}
\label{sec:results}

Before arriving at \S\ref{sec:checkpoint} we ran the same recipe against the
\textsc{Released} checkpoint, with AdamW in place of the Muon pairing and no
weight average. It is worth reporting in full, because every signal available
during training said it was working.

We evaluate by outpainting: $32$\,s of real audio is given as context and the
model generates the following $158$\,s, at the eight sampler steps the released
checkpoint is post-trained for. Quality is the mean absolute difference between
the generated region's $24$-band log-spectral envelope and that of the true
continuation, plus the ratio of generated RMS to the true continuation's. Five
contexts are drawn from widely separated points of a $26.4$\,h source.
...
\midrule
\textsc{Released}, untouched & \textbf{0.279} & 1.28$\times$ \\
$+\,2000$ MSE steps & 0.296 & 1.28$\times$ \\
$+\,6000$ MSE steps & 0.349 & 0.72$\times$ \\
\midrule
Blend $\alpha=0.25$   & 0.296 & 1.28$\times$ \\
Blend $\alpha=0.50$   & 0.324 & \textbf{1.08}$\times$ \\
\bottomrule
\end{tabular}
\caption{Outcome of squared-error training on the post-trained checkpoint.
Quality degrades monotonically with how much of the update is applied, and
interpolating back toward the released weights recovers level without ever
beating them on envelope.}
\label{tab:results}
\end{table}

Three ablations exclude the obvious explanations. It is \textbf{not dosage}:
stopping at $2000$ steps reduces but does not remove the effect. It is \textbf{not
the gates}: freezing every 1-D parameter --- the RMSNorm gains and adaLN
modulation vectors that globally rescale each block --- leaves the level ratio at
$0.58\times$ against $0.59\times$, so the drift is in the weight matrices, in the
velocity field itself. And it is \textbf{not recoverable by interpolation}: a
weight blend trades one defect for the other along a curve that never reaches the
baseline.

The sampler-step sweep is what points at the mechanism. The released checkpoint
degrades gracefully as steps increase ($0.412 \rightarrow 0.577$ from $8$ to
$64$); the finetuned one collapses ($0.221 \rightarrow 2.908$, output falling to
$0.017$ RMS with spectral centroid rising to $9.2$\,kHz --- hiss). A model whose
few-step behaviour is intact but whose many-step behaviour explodes is not a
broken model; it is a model being sampled by the wrong solver family. Table~\ref{tab:probe}
then measures the cause directly.

Throughout all of this, validation loss fell monotonically, from $1.31$ to
$1.01$, and the context-reconstruction term fell twenty-fold. The training curves
were indistinguishable from a healthy run.
...
offsets drawn from the energy gate, two noise seeds each, with both models placed
on \emph{identical trajectories} --- ping-pong redraws noise inside its own loop,
so seeding the initial latent alone is not sufficient.
...
Two results in Table~\ref{tab:final} are worth separating. The first is the
expected one: the finetune moves the model toward the target domain. The second
is that \emph{the merged eight-step model beats the fifty-step model it was
trained as}, $0.2978$ against $0.3817$. Post-trained weights sampled in eight
steps are simply a better generator than base weights sampled in fifty, and the
transplant makes that available to a finetune trained under the only objective
the base weights admit.

The spread column is the practical finding. The released weights produced one
render at $0.47\times$ level, a partial collapse; the merged model's worst case
is $0.96\times$, with a third of the variance. Adding a domain finetune to an
adversarially post-trained model made it not only closer on average but markedly
more reliable.
...
The headline lesson is worth stating plainly: a finetune can be faithful to its
reference implementation, train stably, and lower its loss while making the model
worse, and the loss curve cannot tell you. Ours fell from $1.31$ to $1.01$ over
exactly the interval in which the model was being converted from a sample
predictor into a mean predictor. Any evaluation that had stopped at the scalar
would have reported success.

The generalisable part is the diagnostic, not the incident. Whenever a sampler
rebuilds its state from a one-step estimate --- ping-pong, restart sampling,
consistency-model sampling --- that estimate must be a sample and not a mean, and
the distinction is measurable in one forward pass by comparing the estimate's
scale to its correlation with the truth. The two coincide for a mean predictor
and separate for a sample predictor. We would run this before and after any
training that touches a distilled or post-trained checkpoint.
...
distilled checkpoint. Code is available at \url{https://github.com/mrcolo/stems-loop}.
